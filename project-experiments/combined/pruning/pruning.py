...
def instrument_model(model):
    ...
    if hasattr(base, "bert"):
        ...
        base = base.roberta
    elif hasattr(base, "vit"):
        base = base.vit
        
    layers = getattr(base.encoder, "layer", [])

    # Fetch the config dictionary (Safe across deepcopies)
    config_pruned = getattr(base.config, "pruned_heads", {})
    ...
    for i, layer in enumerate(layers):
        ...
        # USE THE CONFIG INSTEAD OF THE MODULE STATE
        pruned_heads = set(config_pruned.get(i, []))
        
        original_num_heads = num_heads + len(pruned_heads)
        
        # Figure out which original heads are still active
        ...
        def fwd_hook(module, inputs, output, prof=profiler):
            ...
            if context_layer.requires_grad:
                ...
                def bwd_hook(grad):
                    with torch.no_grad():
                        act_reshaped = act.view(batch_size, -1, prof.num_heads, prof.head_size)
                        grad_reshaped = grad.view(batch_size, -1, prof.num_heads, prof.head_size)
                ...
                with torch.no_grad():
                    act_reshaped = context_layer.view(batch_size, -1, prof.num_heads, prof.head_size)
...
def calibrate_with_survival(
    ...
    pbar = tqdm(loader, total=min(len(loader), n_batches), desc="Calibrating (P_e)", leave=False)
    for i, b in enumerate(pbar):
        ...
        for th in thresholds:
            ...
            previously_exited = torch.cat(
                [torch.zeros(batch_size, 1, device=device, dtype=torch.bool), exited[:, :-1]], dim=1
            )
    ...
    if total_samples > 0:
        survival_probs = (layer_survival_counts / (total_samples * len(thresholds))).cpu().tolist()
    else:
        survival_probs = [1.0] * len(layer_survival_counts) if layer_survival_counts is not None else []
...
def decide_heads_to_prune(imp_modules, survival_probs, keep_ratio=0.5):
    ...
    for i, mod_id in enumerate(sorted(imp_modules.keys())):
        ...
        for rel_h, s in enumerate(scores):
            # Map relative index back to the absolute original head index
    ...
    total = len(all_heads)
    import math
    n_drop = max(1, math.ceil(total * (1.0 - keep_ratio)))
    n_keep = total - n_drop

    # 1. Force pick the single best head from each layer as a baseline safety net
    forced_kept = []
    ...
    # 2. Calculate how many remaining slots we have to fill
    remaining_to_keep = n_keep - len(forced_kept)

    # 3. Fill the rest from the highest scoring available heads
    if remaining_to_keep > 0:
        ...
        # If we are forced to keep more than n_keep (e.g. hitting the 1-per-layer floor)
        kept = forced_kept

    # 4. Convert what we KEEP into what we PRUNE
