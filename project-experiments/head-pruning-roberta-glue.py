...
def make_loader(split, bs, shuffle=False, use_fixed_padding=False):
...
def fine_tune(model, train_loader, eval_loader, epochs=1, lr=2e-5, device=DEVICE):
    ...
    scheduler = torch.optim.lr_scheduler.LambdaLR(
        opt, lambda step: min(1.0, step / warmup_steps) * max(0.0, 1.0 - step / total_steps)
    )
    for ep in range(epochs):
        ...
        for b in tqdm(train_loader, desc=f"Epoch {ep+1}"):
            ...
            loss = model(**b)["loss"]
            loss.backward(); opt.step(); scheduler.step(); opt.zero_grad()
